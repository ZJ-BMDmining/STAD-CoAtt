...
class Trainer():
	...
	def train(self):
		...
		_, nodes_embs = self.run_epoch(self.splitter.train, self.args.num_epochs, 'TRAIN', grad=False)
		pd.DataFrame(np.array(nodes_embs.cpu())).to_csv('final.csv', header=None, index=None)
